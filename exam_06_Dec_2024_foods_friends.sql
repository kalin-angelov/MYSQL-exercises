...
-- 5 Get all offerings from restaurant

select o.name, price
from offerings as o
join restaurants as r
on o.restaurant_id and r.id
where r.name = 'Burger Haven'
order by o.id
limit 3;

-- 6 Get all customers without orders
...
-- 7 Get all offerings from orders of the customer

select o.id, o.name
from offerings as o
join orders as ord
on ord.restaurant_id = o.restaurant_id
join customers as c
on c.id = ord.customer_id
where o.vegan is false 
and concat_ws(' ', c.first_name, c.last_name) = 'Sofia Sanchez'
order by o.id;

-- 8 Get all restaurants with regular customers

select r.id, r.name
from restaurants as r
join orders as ord
on r.id = ord.restaurant_id
join offerings as o
...
and o.vegan is true
and ord.priority = 'HIGH'
group by r.id, r.name
order by r.id;

-- 9 Offering price categories
