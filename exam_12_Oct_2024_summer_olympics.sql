...
concat_ws(' ', last_name, 'comes from', c.name) as last_name, 
(age + c.id) as age, 
country_id
from athletes as a
join countries as c
...
join sports as s
on d.sport_id = s.id
group by a.first_name, a.last_name, a.id, s.name
...
create function udf_total_medals_count_by_country (input VARCHAR(40)) 
returns int deterministic
begin
	return (
		select count(dam.medal_id) as medals_count
		from athletes as a
		join countries as c
		on a.country_id = c.id
		join disciplines_athletes_medals as dam
...
create procedure udp_first_name_to_upper_case(input char(1))
begin
	update athletes
    set first_name = upper(first_name)
    where first_name like concat('%', input);
end//
